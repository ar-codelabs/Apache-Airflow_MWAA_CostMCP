...
> 참고: `mcp_config.json`은 Claude Desktop 연동용 설정 파일입니다. Streamlit 챗봇은 코드 내부에서 직접 uvx를 실행하므로 이 파일과 무관하게 동작합니다.
...
python-dotenv      # 환경 변수
psutil             # 프로세스 관리
nest-asyncio       # asyncio 중첩 허용
strands-agents     # Strands Agent 프레임워크
strands-agents-tools  # Strands 도구 모음
```
...
```
@cmdb-server get_identity_policies
@cmdb-server search_resources query="cloudwatch"
```
